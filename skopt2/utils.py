...
def dump(res, filename, store_objective=True, **kwargs):
    """
    Store an skopt2 optimization result into a file.
    ...
    store_objective : boolean, default=True
        ...
        critical, one can delete it before calling `skopt2.dump()` and thus
...
def load(filename, **kwargs):
    """
    Reconstruct a skopt2 optimization result from a file
    persisted with skopt2.dump.

    .. note::
        Notice that the loaded optimization result can be missing
        the objective function (`.specs['args']['func']`) if `skopt2.dump`
...
def expected_minimum(res, n_random_starts=20, random_state=None):
    ...
    ----------
    res : `OptimizeResult`, scipy object
        The optimization result returned by a `skopt2` minimizer.
...
def cook_initial_point_generator(generator, **kwargs):
    ...
    generator : "lhs", "sobol", "halton", "hammersly", "grid", "random" \
        ...
        Should inherit from `skopt2.sampler.InitialPointGenerator`.
...
def dimensions_aslist(search_space):
    ...
    search_space : dict
        ...
        :class:`skopt2.space.Dimension` (Real, Integer or Categorical)

    Returns
    -------
    params_space_list: list
        list of skopt2.space.Dimension instances.

    Examples
    --------
    >>> from skopt2.space.space import Real, Integer
    >>> from skopt2.utils import dimensions_aslist
...
def point_asdict(search_space, point_as_list):
    ...
    .. seealso:: :class:`skopt2.utils.point_aslist`
    ...
    --------
    >>> from skopt2.space.space import Real, Integer
    >>> from skopt2.utils import point_asdict
    >>> search_space = {'name1': Real(0,1),
    ...                 'name2': Integer(2,4), 'name3': Real(-1,1)}
...
def point_aslist(search_space, point_as_dict):
    ...
    .. seealso:: :class:`skopt2.utils.point_asdict`
    ...
    --------
    >>> from skopt2.space.space import Real, Integer
    >>> from skopt2.utils import point_aslist
    >>> search_space = {'name1': Real(0,1),
    ...                 'name2': Integer(2,4), 'name3': Real(-1,1)}
...
def use_named_args(dimensions):
    ...
    >>> from skopt2.space import Real
    >>> from skopt2 import forest_minimize
    >>> from skopt2.utils import use_named_args
