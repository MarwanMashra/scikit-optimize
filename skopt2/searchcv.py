...
class BayesSearchCV(BaseSearchCV):
    ...
    Parameters are presented as a list of skopt2.space.Dimension objects.
    ...
    search_spaces : dict, list of dict or list of tuple containing (dict, int).
        ...
        and values are skopt2.space.Dimension instances (Real, Integer
        or Categorical) or any other valid value that defines skopt2
        dimension (see skopt2.Optimizer docs). Represents search space
    ...
    >>> from skopt2 import BayesSearchCV
    >>> # parameter ranges are specified by one of below
    >>> from skopt2.space import Real, Categorical, Integer
    >>>
    ...
    def _make_optimizer(self, params_space):
        """Instantiate skopt2 Optimizer class.

        Parameters
        ----------
        params_space : dict
            Represents parameter search space. The keys are parameter
            names (strings) and values are skopt2.space.Dimension instances,
